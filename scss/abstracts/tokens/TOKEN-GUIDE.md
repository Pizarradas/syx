...
4. Organisms expansion (roadmap)
5. Public documentation site (roadmap)
...
> **Note**: `!important` is avoided in SYX components by relying on `@layer`. The only accepted uses are:
>
> - `animation: none !important` inside `@media (prefers-reduced-motion)` — required by the spec
> - `padding: 0 !important` / `gap: 0 !important` inside grid modifier `--no-pad` — required to override grid specificity
>
