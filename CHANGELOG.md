...
- **El puente a Figma: `get_figma_spec`, `npm run export:figma` y `scripts/lib/figma.js`.** SYX ya hablaba con herramientas que no son SYX —`export:tokens` emite W3C DTCG desde 4.12.0— pero no con Figma, y el motivo es más concreto que «falta un exportador»: una variable COLOR de Figma guarda `{r,g,b,a}` numéricos, y este sistema está escrito entero en `oklch()`. Un fichero DTCG con `oklch(0.498 0.282 266.24)` se importa como texto o no se importa. **La frontera real no es un formato: es una conversión**, y por eso vive en una función y no en una plantilla.
  ...
  `lib/figma.js` la cruza en las dos direcciones que hacían falta: oklch → oklab → LMS → sRGB con gamma, sin dependencias —meter una librería de color en un paquete cuyo argumento es no arrastrar nada habría sido contradecirse por cuarenta líneas—, `rem` → píxeles con la raíz en 16 porque SYX no declara `font-size` en `html`, y el salto que faltaba de verdad: **de nombre de token a propiedad de nodo**. `--component-button-border-radius` no le dice nada a un agente que está creando un frame; `cornerRadius: 4` sí.

- **`get_figma_spec`, la undécima herramienta del servidor MCP.** `get_component` devuelve nombres de token, y un nodo de Figma necesita números. Sin esta consulta, el agente que dibuja traduce por su cuenta entre lo que SYX dice y lo que Figma acepta — que es exactamente donde nace la desviación que `scan_for_drift` existe para cazar, solo que en el otro lado del puente y sin nadie mirando.
...
- **`npm run export:figma` — 7 ficheros, 3635 variables, 1879 omitidas con motivo.** Dos colecciones por tema (`SYX · Semantic`, `SYX · Component`) con sus dos modos, y los 34 componentes con sus propiedades. Tres decisiones que conviene no descubrir a mitad de una importación:
  ...
  · **Un fichero por tema, no uno con catorce modos.** Los modos por colección están limitados por plan —cuatro en Professional—, así que un solo fichero con siete temas por dos modos sería inservible en casi cualquier cuenta. Un tema es una biblioteca; sus dos modos son claro y oscuro.

  · **Lo que no se traduce se lista.** Unos 250 tokens por tema se quedan fuera: tipografía fluida en `clamp()`, `color-mix()`, unidades relativas, sombras, degradados, SVG embebidos. Cada uno aparece en `omitidos` con su motivo, y el motivo dice dónde SÍ está la cosa en Figma cuando la hay —una sombra es un efecto, un degradado es un fill, una imagen es un asset—. Exportar 543 y callar los 247 descartados sería mentir por omisión, que es el error que este repositorio ya cometió una vez con `export:tokens` antes de arreglarlo.

- **`npm run check:figma`, dentro de la cadena de `npm run check`.** Misma forma que `check:tokens` y `check:registry`: regenera y falla si lo versionado está desfasado, ignorando la versión al comparar —subir de versión no desfasa una exportación cuyos valores no han cambiado, y hacer fallar al guardián por eso enseña a ignorarlo. `check-mcp.js` pasa de 10 a 11 herramientas y añade el caso que importa: que lo que sale sean números y no CSS, que cambie con el modo, y que ningún STRING acabe en una propiedad que espera pintura.

- **`_agents/workflows/export-to-figma.md`, el quinto workflow.** Los siete pasos en el orden en que tienen que pasar, y —lo que de verdad hacía falta— los tres sitios donde alguien se equivocaría solo: **atar a la variable en vez de pegar el valor** (un valor pegado es correcto en claro y falso en oscuro, y nada te lo dice), **no arreglar** la ausencia de primitivos porque es R01 y no un olvido, y **no tocar a mano** `contracts/figma/`, que es derivado. Listado en `CLAUDE.md`, `AGENTS.md` y `AI_GUIDELINES.md`.

- **La séptima regla de `AI_GUIDELINES.md`: nunca convertir un valor a mano para Figma.** Deducir que `oklch(0.498 0.282 266.24)` es más o menos `#1e3aff` es exactamente cómo una biblioteca empieza a desviarse de su propio sistema. Hay una conversión, y está en `scripts/lib/figma.js`.

- **Documentado en las páginas, no solo en los `.md`.** `docs.html` gana el apartado **7 · Out to Figma** dentro del bloque agentic —los seis primeros son el agente operando el sistema por dentro; este es el mismo sistema apuntando hacia fuera— con la tabla de equivalencias, las cifras medidas y el límite de Code Connect. `home.html` gana su tarjeta en *AI First*. Las clases usadas se contrastaron con `npm run scan`: cero fantasmas nuevos.

- **`contracts/figma/` se versiona; `contracts/dtcg/` no.** Los dos son derivados y los dos pesan unos 3 MB, así que la diferencia no es el tamaño: es el guardián. La exportación de Figma tiene `check:figma` dentro de `npm run check` y no puede desfasarse sin que CI se ponga rojo; la de DTCG no tiene ninguno, y un artefacto derivado que nadie comprueba se pudre solo. Si algún día cae el guardián, cae con él la decisión de versionar.

- **`listTokens` en la capa de consulta, y deliberadamente NO como herramienta MCP.** Devuelve más de mil pares: servírselo a un agente sería justo lo que el servidor existe para evitar. Existe para los exportadores, que necesitan recorrerlo todo y que, sin ella, volverían a escribir la cascada `base → light → dark` por su cuenta — que es como acaban divergiendo dos copias del mismo criterio.
...
- **Una identidad tiene que ser construible con SYX, o no es una decisión.** Es la condición que le faltaba al modo y la que justifica que exista dentro de este repositorio y no en un documento aparte. BRAND cierra ahora con una sección *Landing it in SYX* y con un apartado **Buildability** obligatorio en la respuesta: cada eje resuelve a familias de tokens que existen, aterriza en un solo escalón y sobrevive a R01–R04 —comprobado con `get_token`, `classify_change` y `validate_snippet` **antes** de entregar, no después.

  El hecho arquitectónico sobre el que se apoya ya estaba en el sistema: **R01 prohíbe `var(--primitive-*)` en un componente**, así que todo átomo, molécula y organismo lee solo `--component-*` y `--semantic-*`. Por eso una identidad se *instala*: se mueve la capa de tema y el árbol entero se repinta, en todos los bundles, sin tocar una línea de SCSS de componente. De ahí la prueba dura que el modo hereda: **una identidad que exige editar SCSS de componente no es una identidad, es un rediseño, y hay que llamarlo así.**

  Con las dos consecuencias escritas: un eje que no tiene familia de tokens donde aterrizar **se enruta a TOKEN**, nunca se inventa dentro de un tema —*una identidad que se acuña su propio vocabulario es drift con un manual de marca encima*—; y una página construida sobre una identidad BRAND **no debe contener un solo valor escrito a mano**. Si necesita un hex, un `px` crudo o una regla a medida, ha encontrado un token que falta o un componente que falta, y las dos cosas suben por la cadena en vez de resolverse en local. `scan_for_drift` es quien lo mide.

- **El pipeline llega hasta el producto, y sus dos pasos centrales son condicionales.** `BRAND → THEME → TOKEN → UI → producto`, pero TOKEN y UI solo corren si un eje necesitó un nombre o un componente que no existía. **La mayoría de las identidades se los saltan, y eso es el sistema funcionando, no el pipeline quedándose corto.** El flujo de `_agents/modes/README.md` añade además los dos pasos que convierten un documento en algo real: `npm run build && npm run validate`, y `scan_for_drift` sobre las páginas.
